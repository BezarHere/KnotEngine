...
namespace indir
{
  ...
  struct MoveCTor
  {
    inline MoveCTor(void *_origin) : origin{ _origin } {}

    template <class T>
    inline void operator()(T &val) const noexcept {
      new (&val) T(std::forward<T>(*reinterpret_cast<T *>(origin)));
    }
  ...
  struct CopyCTor
  {
    inline CopyCTor(void *_origin) : origin{ _origin } {}

    template <class T>
    inline void operator()(T &val) const noexcept {
      new (&val) T(*reinterpret_cast<const T *>(origin));
    }
  ...
  struct MoveAssign
  {
    inline MoveAssign(void *_origin) : origin{ _origin } {}

    template <class T>
    inline void operator()(T &val) const noexcept {
      val = std::forward<T>(*reinterpret_cast<T *>(origin));
    }
  ...
  struct CopyAssign
  {
    inline CopyAssign(void *_origin) : origin{ _origin } {}

    template <class T>
    inline void operator()(T &val) const noexcept {
      val = *reinterpret_cast<const T *>(origin);
    }
